...
const Stage1Tab = forwardRef(({ jobId, job, template, setViewStage }, ref) => {
  const { data, setData, loading, saving, save } = useStageData(jobId, 1);
  ...
  React.useEffect(() => {
    if (data && !hasInitialized.current) {
      ...
      if (data.inspectionDecision && !window.__stage1_edit_mode_active) {
  ...
  const handleSave = async () => {
    try {
      ...
      window.__stage1_edit_mode_active = false; // Reset edit flag on manual save
      setIsSummaryMode(true);
  ...
  const handleTestPhotoUpload = async (testName, e) => {
    ...
    try {
      const urls = await uploadPhotos(e.target.files);
      if (urls.length > 0) {
        setTestField(testName, 'photo', urls[0]);
  ...
  const getChecked = (section, item) => {
    const v = data?.[section]?.[item];
    return typeof v === 'object' ? (v?.checked || false) : !!v;
  };
  const getDate = (section, item) => {
    const v = data?.[section]?.[item];
    return typeof v === 'object' ? (v?.date || '') : '';
  };

  const setTestField = (testName, field, value) => {
    setData(prev => {
      const test = prev?.electricalTests?.[testName] || {};
      const updated = { ...test, [field]: value };
      // Auto-evaluate status
      const def = template?.stage1?.electricalTests?.find(t => t.name === testName);
      if (def) {
      ...
      return { ...prev, electricalTests: { ...(prev.electricalTests || {}), [testName]: updated } };
    });
  };

  const setPartsField = (partName, field, value) => {
    setData(prev => ({
      ...prev,
      partsChecklist: {
        ...(prev.partsChecklist || {}),
        [partName]: { ...(prev.partsChecklist?.[partName] || {}), [field]: value }
      }
    }));
  };
  ...
  const s1 = template?.stage1 || {};
  const surgeTests = s1.surgeTests || [];
  const sensorTests = s1.sensorTests || [];
    ...
    return (
      <InspectionSummaryView 
        ...
        onEdit={() => {
          ...
          // Set window flag so it doesn't auto-revert to summary if data changes
          window.__stage1_edit_mode_active = true;
        }}
      />
    );
  }
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest">Overall Photos</h3>
            <p className="text-xs text-slate-400 mt-0.5">Before, during, after inspection</p>
          </div>
        ...
        {data.photos?.length > 0 ? (
          ...
          <div className="border-2 border-dashed border-slate-200 rounded-xl py-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">
            🖼️ No photos yet
          </div>
        )}
      </div>
      ...
      {/* Electrical Tests */}
      {s1.electricalTests?.length > 0 && (
        <div className="space-y-6">
          {categorizeElectricalTests(s1.electricalTests).map(([categoryName, tests]) => (
                        ...
                        return (
                          <React.Fragment key={i}>
                            ...
                            <tr className="hover:bg-slate-50">
                              ...
                              <td className="py-2.5 pr-4 text-slate-500 text-xs">{test.standardValue}</td>
                          {tests.some(t => t.hasAppliedVoltage) && (
                            <td className="py-2.5 pr-4">
                              {test.hasAppliedVoltage ? (
                                <input type="number" className="w-20 px-2 py-1 text-xs border border-slate-200 rounded-lg outline-none"
                                  placeholder="V" value={row.appliedVoltage || ''} onChange={e => setTestField(test.name, 'appliedVoltage', e.target.value)} />
                          ...
                          <td className="py-2.5 pr-4">
                            <div className="flex items-center gap-1">
                              <input type="number" step="any" className="w-20 px-2 py-1 text-xs border border-slate-200 rounded-lg outline-none"
                                placeholder="Value" value={row.value || ''} onChange={e => setTestField(test.name, 'value', e.target.value)} />
                              <select className="px-1 py-1 text-xs border border-slate-200 rounded-lg bg-white outline-none font-semibold text-slate-600"
                                value={currentUnit} onChange={e => setTestField(test.name, 'unit', e.target.value)}>
                            ...
                            {row.photo ? (
                              <div className="relative inline-block group">
                                ...
                                <button className="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity"
                                  onClick={() => setTestField(test.name, 'photo', '')}>✕</button>
                              </div>
                            ) : (
      ...
      {/* Parts Checklist */}
      {s1.partsChecklist?.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest mb-4">Part Check List</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-2 text-xs font-bold text-slate-500 uppercase tracking-widest w-8">#</th>
                  <th className="text-left py-2 pr-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Part Name</th>
                  <th className="text-left py-2 pr-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Part No</th>
                  <th className="text-center py-2 pr-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Qty</th>
                  <th className="text-left py-2 pr-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Status</th>
                  <th className="text-left py-2 text-xs font-bold text-slate-500 uppercase tracking-widest">Remark</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {s1.partsChecklist.map((part, i) => {
                  const row = data.partsChecklist?.[part.partName] || {};
                  const isAvail = row.status === 'Available';
                  const isNotAvail = row.status === 'Not Available';
                  return (
                    <tr key={i} className={`hover:bg-slate-50 ${isNotAvail ? 'bg-red-50' : ''}`}>
                      <td className="py-2 text-xs text-slate-400">{i + 1}</td>
                      <td className="py-2 pr-4 font-semibold text-slate-700">{part.partName}</td>
                      <td className="py-2 pr-4 text-xs text-slate-400 font-mono">{part.partNo || '—'}</td>
                      <td className="py-2 pr-4 text-center text-xs text-slate-600">{part.quantity}</td>
                      <td className="py-2 pr-4">
                        <div className="flex gap-2">
                          <button onClick={() => setPartsField(part.partName, 'status', 'Available')}
                            className={`px-2 py-1 rounded text-xs font-bold border transition-all ${isAvail ? 'bg-green-500 text-white border-green-500' : 'bg-white text-slate-500 border-slate-200 hover:border-green-300'}`}>
                            ✓ Available
                          </button>
                          <button onClick={() => setPartsField(part.partName, 'status', 'Not Available')}
                            className={`px-2 py-1 rounded text-xs font-bold border transition-all ${isNotAvail ? 'bg-red-500 text-white border-red-500' : 'bg-white text-slate-500 border-slate-200 hover:border-red-300'}`}>
                            ✗ Not Available
                          </button>
                        </div>
                      </td>
                      <td className="py-2">
                        <input type="text" className="w-full px-2 py-1 text-xs border border-slate-200 rounded-lg outline-none"
                          placeholder="Remark..." value={row.remark || ''} onChange={e => setPartsField(part.partName, 'remark', e.target.value)} />
                      </td>
                    </tr>
                  );
                })}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        <textarea rows={3} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none"
          placeholder="Any remarks on the incoming condition..." value={data.overallRemarks || ''} onChange={e => setData(p => ({ ...p, overallRemarks: e.target.value }))} />
      </div>

      {/* Inspection Decision */}
      <div className="bg-white rounded-2xl border-2 border-blue-100 shadow-sm p-6">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white text-base">⚖️</div>
          <div>
            <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest">Inspection Decision</h3>
            <p className="text-xs text-slate-400 mt-0.5">Select the next course of action for this component</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
          {[
            {
              key: 'Proceed to Overhaul',
              icon: '🔧',
              color: 'green',
              title: 'Proceed to Overhaul',
              desc: 'Component is ready for full dismantling and overhaul process'
            },
            {
              key: 'Send to Vendor',
              icon: '🏭',
              ...
              desc: 'Winding or critical issue — send to external specialist vendor'
            },
            {
              key: 'On Hold',
              icon: '⏸️',
              color: 'red',
              title: 'On Hold',
              desc: 'Component has issues — hold pending further assessment or decision'
            }
          ].map(({ key, icon, color, title, desc }) => {
            const isSelected = data.inspectionDecision === key;
            const colorMap = {
              green: {
                selected: 'border-green-500 bg-green-50 ring-2 ring-green-200',
                unselected: 'border-slate-200 bg-slate-50 hover:border-green-300 hover:bg-green-50/50',
                badge: 'bg-green-500',
                text: 'text-green-800',
                sub: 'text-green-600'
              },
              amber: {
                ...
                unselected: 'border-slate-200 bg-slate-50 hover:border-amber-300 hover:bg-amber-50/50',
                badge: 'bg-amber-500',
                text: 'text-amber-800',
                sub: 'text-amber-600'
              },
              red: {
                ...
                unselected: 'border-slate-200 bg-slate-50 hover:border-red-300 hover:bg-red-50/50',
                badge: 'bg-red-500',
                text: 'text-red-800',
                sub: 'text-red-600'
              }
            ...
            return (
              <button
                key={key}
                onClick={() => setData(p => ({ ...p, inspectionDecision: isSelected ? '' : key }))}
                className={`relative w-full text-left p-4 rounded-xl border-2 transition-all duration-200 ${isSelected ? c.selected : c.unselected}`}
              >
                {isSelected && (
                  <div className={`absolute top-3 right-3 w-5 h-5 rounded-full ${c.badge} flex items-center justify-center`}>
                ...
                <div className={`font-black text-sm mb-1 ${isSelected ? c.text : 'text-slate-700'}`}>{title}</div>
                <div className={`text-xs leading-relaxed ${isSelected ? c.sub : 'text-slate-400'}`}>{desc}</div>
              </button>
            );
          })}
        </div>
        {data.inspectionDecision && (
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest block mb-2">
              Reason / Notes for "{data.inspectionDecision}"
            </label>
            <textarea rows={2} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none"
              placeholder={`Explain why the decision is "${data.inspectionDecision}"...`}
              value={data.inspectionDecisionReason || ''}
              onChange={e => setData(p => ({ ...p, inspectionDecisionReason: e.target.value }))} />
          </div>
        )}
        {!data.inspectionDecision && (
          <div className="flex items-center gap-2 px-4 py-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
            <span className="text-slate-400 text-lg">👆</span>
            <span className="text-xs text-slate-400 font-medium">Select a decision above to complete the inspection assessment</span>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-6 right-6">
        <button onClick={handleSave} disabled={saving} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all disabled:opacity-60 flex items-center gap-2">
          {saving ? '💾 Saving...' : '💾 Save Stage 1'}
        </button>
      </div>
...
Stage1Tab.displayName = 'Stage1Tab';
export default Stage1Tab;
